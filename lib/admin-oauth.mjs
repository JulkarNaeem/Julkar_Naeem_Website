...
export class AdminOAuthError extends Error {
  constructor(stage, cause) {
    super(`Admin OAuth failed during ${stage}.`, { cause });
    this.name = "AdminOAuthError";
    this.stage = stage;
    this.code = typeof cause?.code === "string" ? cause.code : undefined;
  }
}

async function oauthStage(stage, work) {
  try { return await work(); }
  catch (error) { throw new AdminOAuthError(stage, error); }
}

function validateGitHubAuthorizationResponse(server, client, callbackUrl, expectedState) {
  const received = callbackUrl instanceof URL ? callbackUrl.searchParams : callbackUrl;
  // GitHub's documented web flow returns only code and state. Some GitHub
  // responses also include an issuer value that is not part of that contract
  // and may differ only by URL serialization. State was already checked against
  // the encrypted transaction cookie; rebuild the standard response from the
  // documented parameters before handing it to oauth4webapi.
  const codes = received.getAll("code"), states = received.getAll("state");
  if (codes.length !== 1 || !codes[0] || states.length !== 1 || states[0] !== expectedState
    || received.has("error") || received.has("response") || received.has("id_token") || received.has("token")) {
    const error = new Error("GitHub authorization response is invalid.");
    error.code = "OAUTH_INVALID_RESPONSE";
    throw error;
  }
  return oauth.validateAuthResponse(server, client, new URLSearchParams({ code: codes[0], state: states[0] }), expectedState);
}
...
export async function finishOAuth(config, transaction, callbackUrl, transport = fetch) {
  ...
  const params = await oauthStage("authorization-response", () => config.provider === "github"
    ? validateGitHubAuthorizationResponse(server, client, callbackUrl, transaction.state)
    : oauth.validateAuthResponse(server, client, callbackUrl, transaction.state));
  const response = await oauthStage("token-request", () => oauth.authorizationCodeGrantRequest(server, client, oauth.ClientSecretPost(config.clientSecret),
    params, transaction.redirectUri, transaction.verifier, options));
  const result = await oauthStage("token-response", () => oauth.processAuthorizationCodeResponse(server, client, response,
    config.provider === "google" ? { expectedNonce: transaction.nonce, requireIdToken: true } : {}));
  let identity;
  if (config.provider === "google") {
    await oauthStage("id-token-signature", () => oauth.validateApplicationLevelSignature(server, response, options));
    identity = oauth.getValidatedIdTokenClaims(result);
  } else {
    const profile = await oauthStage("identity-request", () => oauth.protectedResourceRequest(result.access_token, "GET", new URL("https://api.github.com/user"),
      new Headers({ Accept: "application/vnd.github+json", "User-Agent": "Julkar-Naeem-Website-Control-Panel" }), null, options));
    if (!profile.ok) throw new Error("Provider identity unavailable.");
    identity = await oauthStage("identity-response", () => profile.json());
  }
